...
impl DeepseekClient {
    ...
    pub async fn generate_dataset_with_prompt(&self, task: &TaskDefinition, count: usize) -> anyhow::Result<Vec<serde_json::Value>> {
        ...
        // สมมติว่า output อยู่ที่ resp_json["choices"][0]["message"]["content"]
        let content = resp_json["choices"][0]["message"]["content"].as_str().unwrap_or("");
        // พยายาม parse เป็น JSON array
        let parsed: serde_json::Value = serde_json::from_str(content.trim()).unwrap_or(serde_json::Value::Null);
